...
function generatePlaceholderImage(width, height, text, category, index) {
    ...
    const labels = {
        portrait: ['回眸一瞬', '光影之间', '沉静如海', '微笑温度', '灵魂凝视', '风中姿态'],
        wilderness: ['晨曦山脊', '云海翻涌', '荒原落日', '草甸之风', '远山呼唤', '寂静山谷'],
        architecture: ['都市剪影', '几何之光', '玻璃幻境', '明暗交错', '空间韵律', '光束漫舞'],
...
function generateAvatar() {
    ...
    ctx.lineWidth = 2;
    ctx.strokeRect(230, 480, 180, 120);
    ctx.beginPath();
    ctx.arc(320, 540, 40, 0, Math.PI * 2);
    ctx.stroke();
    ...
    ctx.textAlign = 'center';
    ctx.fillText('许墨尘。', 320, 720);

    ctx.fillStyle = 'rgba(255, 255, 255, 0.3)';
    ctx.font = '300 18px "Inter", sans-serif';
    ctx.fillText('PHOTOGRAPHER', 320, 755);
...
function renderAllImages() {
    const categories = ['portrait', 'wilderness', 'architecture', 'life'];
    const categoryNames = {
        portrait: '人像',
        wilderness: '遐想',
        architecture: '光影',
    ...
    categories.forEach(cat => {
        ...
        for (let i = 0; i < 6; i++) {
            const realPath = `images/${cat}/0${i + 1}.jpg`;
